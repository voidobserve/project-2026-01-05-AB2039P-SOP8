...
*
*Copyright (c) 2024-08-15, BLUETRUM
******************************************************************************************************************************
**/
...
uint16_t irtx_nec_txdata_encode(uint8_t *dest, uint8_t address, uint8_t cmd)
{
    uint16_t cnt = 0;
    uint16_t offset = 0;
...
uint16_t irtx_nec_repeat_txdata_encode(uint8_t *dest)
{
    uint16_t offset = 0;
    //repeat code
...
void irtx_example(void)
{
    ...
    if (tick_check_expire(irtx_send_tick, 110)) {
        ...
        if (irtx_repeat_test_flag == 0) {
            ...
            len = irtx_nec_txdata_encode(tx_buf, 0x4E, 0x1F);
            printf("irtx\n");
        } else {
            len = irtx_nec_repeat_txdata_encode(tx_buf);
            printf("repeat\n");
        }

        len /= 2;
        irtx_dma_kick((uint32_t)&tx_buf, len-1);  //len unit:2byte
        irtx_kick();
    }
}
