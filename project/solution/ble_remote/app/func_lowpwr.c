...
void bsp_gpio_wakeup_config(uint8_t lp_type)
{
    ...
    config.gpio_cfg.edge = WK_EDGE_FALLING;
...
void bsp_gpio_scan_mode_de_save(void)
{
    ...
    GPIOBDE |= iokeyscan_gpiobde & 0xfff;  //PB0~PB11
}
#endif
#endif
...
void bsp_keyscan_lowpwr_mode_wakeup_config(void)
{
    ...
    config.source = WK_LP_GPIO;
    config.gpio_cfg.edge = WK_EDGE_FALLING;
    config.gpio_cfg.gpiox = GPIOB_REG;
    config.gpio_cfg.gpio_pin = GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9;
    config.gpio_cfg.gpio_pupd = GPIO_PUPD_PU10K;
    lowpwr_gpio_wakeup_config(&config);
    gpio_edge_pic_config(gpio_isr_callback, 0, ENABLE);
}
