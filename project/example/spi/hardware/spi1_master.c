...
#if (SPI1_TEST_MODE == SPI1_2WIRE_TEST_MASTER)
// Dev is enabled when CS pin is low level.
void spi1_master_set_cs(FUNCTIONAL_STATE dev_status)
{
...
// SPI1接口设置IO口输入输出方向
void spi1_2wire_master_set_dir(GPIO_DIR_TYPEDEF dir)
{
    if (dir == GPIO_DIR_OUTPUT) {
        gpio_set_dir(SPI1_MASTER_MOSI_PORT, SPI1_MASTER_MOSI_PIN, GPIO_DIR_OUTPUT);
        spi_dir_sel(SPI1, SPI_DIR_TX);
    } else {
        gpio_set_dir(SPI1_MASTER_MOSI_PORT, SPI1_MASTER_MOSI_PIN, GPIO_DIR_INPUT);
        spi_dir_sel(SPI1, SPI_DIR_RX);
    }
}

// SPI1接口获取1Byte数据
uint8_t spi1_2wire_master_read_byte(void)
{
...
// SPI1接口发送1Byte数据
void spi1_2wire_master_write_byte(uint8_t data)
{
...
// 执行spi1_2wire_master例程
void spi1_2wire_master_example(void)
{
    uint8_t write_data[3] = {0x01, 0x02, 0x03};
    uint8_t read_data[3];
    while (1) {
        spi1_master_set_cs(ENABLE);

#if (SPI1_TEST_MASTER_DMA_MODE_EN == 0)
        spi1_2wire_master_set_dir(GPIO_DIR_OUTPUT);
        spi1_2wire_master_write_byte(write_data[0]);
        delay_ms(1);  //给从机一点处理数据的时间
        spi1_2wire_master_write_byte(write_data[1]);
        delay_ms(1);  
        spi1_2wire_master_write_byte(write_data[2]);
        delay_ms(1);  
        spi1_2wire_master_set_dir(GPIO_DIR_INPUT);
        read_data[0] = spi1_2wire_master_read_byte();
        delay_ms(1);  
        read_data[1] = spi1_2wire_master_read_byte();
        delay_ms(1);  
        read_data[2] = spi1_2wire_master_read_byte();
#else
        spi1_2wire_master_set_dir(GPIO_DIR_OUTPUT);
        ...
        while (!spi_get_flag(SPI1, SPI_FLAG_PENDING));  // waiting tx pending
        spi_clear_flag(SPI1, SPI_FLAG_PENDING);
#endif  //(SPI1_TEST_MASTER_DMA_MODE_EN == 0)

        spi1_master_set_cs(DISABLE);

        printf("M:0x%x, 0x%x, 0x%x\n", read_data[0], read_data[1], read_data[2]);
...
// SPI1接口获取1Byte数据
uint8_t spi1_3wire_master_read_byte(void)
{
    spi_send_data(SPI1, 0xff);  //kick
...
// SPI1接口发送1Byte数据
void spi1_3wire_master_write_byte(uint8_t data)
{
    spi_send_data(SPI1, data);  //kick and send data
...
#if (SPI1_TEST_MASTER_DMA_MODE_EN == 0)
        spi1_3wire_master_write_byte(write_data[0]);
        delay_ms(1);  //给从机一些处理数据的时间
        spi1_3wire_master_write_byte(write_data[1]);
        delay_ms(1);
        spi1_3wire_master_write_byte(write_data[2]);
        delay_ms(1);

        read_data[0] = spi1_3wire_master_read_byte();
        delay_ms(1);
        read_data[1] = spi1_3wire_master_read_byte();
        delay_ms(1);
        read_data[2] = spi1_3wire_master_read_byte();
#else
        spi_dir_sel(SPI1, SPI_DIR_TX);
        spi_set_dma_addr(SPI1, (uint32_t)&write_data);
        spi_set_dma_cnt(SPI1, sizeof(write_data));
