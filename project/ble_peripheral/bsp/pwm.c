...
*/
AT(.com_text.isr) void light_module_set_bright(u16 p_liangud)
{
...
*/
AT(.com_text.isr) void light_module_set_temperature(u16 p_wendu)
{
...
static const u16 sos_temperature[] = {
    10000, 10000, 10000, 10000, 10000, 10000,   // S: 冷白色
    10000,                                      // 间隔
    0, 10000, 0, 10000, 0, 10000,               // O: 暖白色
    10000,                                      // 间隔
    10000, 10000, 10000, 10000, 10000, 10000,   // S: 冷白色
    10000,                                      // 间隔
};
AT(.com_text.isr) void light_module_10ms_function(void)
{
...
AT(.com_text.isr) void pwm_driver_10ms_isr(void)
{
...
void light_module_ble_event(u8 *buffer, u16 len)
{
    ...
    {   // 同步
        // 返回: 0x01, 0xF9, 0x02(status), 0x01(power), 0xNN(0:OFF,1:ON)
        // 返回: 0x01, 0xF9, 0x02(status), 0x02(auto), 0xNN(0:OFF,1:ON)
        // 返回: 0x01, 0xF9, 0x03(mode), 0x01(static), 0xNN(0~100:W), 0xNN(0~100:B)
        // 下面指令根据模式选一个返回
        // 返回: 0x01, 0xF9, 0x03(mode), 0x02(weather), 0xNN(index), 0xNN(0~100:W), 0xNN(0~100:B)
        // 返回: 0x01, 0xF9, 0x03(mode), 0x03(flash)
        // 返回: 0x01, 0xF9, 0x03(mode), 0x04(fade)
        // 返回: 0x01, 0xF9, 0x03(mode), 0x05(SOS)
        // 返回: 0x01, 0xF9, 0x03(mode), 0x06(DRL)
        ...
        service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x01, light_cb.temperature_target / 100, light_cb.brightness_target / 100 }, 6);

        if (light_cb.mode == LIGHT_MODE_WEATHER)
            service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x02, light_cb.weather_index, light_cb.temperature_target / 100, light_cb.brightness_target / 100 }, 7);
        else if (light_cb.mode == LIGHT_MODE_FLASH)
            service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x03 }, 4);
        else if (light_cb.mode == LIGHT_MODE_FADE)
            service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x04 }, 4);
        else if (light_cb.mode == LIGHT_MODE_SOS)
            service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x05 }, 4);
        else if (light_cb.mode == LIGHT_MODE_DRL)
            service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x06 }, 4);
    }
    ...
    else if (len == 5 && buffer[2] == 0x02 && buffer[3] == 0x02)
    {   // AUTO模式开关
        // buffer[4](0:OFF,1:ON)
        light_cb.auto_weather_enable = buffer[4] ? 1 : 0;
        service_notify_event((u8 []){ 0x01, 0xF9, 0x02, 0x02, light_cb.auto_weather_enable }, 5);
    }
    else if (len == 6 && buffer[2] == 0x03 && buffer[3] == 0x01)
    {   // 静态模式设置色温亮度
        light_cb.mode = LIGHT_MODE_STATIC;
        param1 = buffer[4] <= 100 ? buffer[4] : 100;
        param2 = buffer[5] <= 100 ? buffer[5] : (buffer[5] >= 1 ? buffer[5] : 1);
        service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x01, param1, param2 }, 6);
        light_cb.temperature_target = param1 * 100;
        light_cb.brightness_target = param2 * 100;
        light_cb.brightness_current = -1;
        light_cb.temperature_current = -1;
    }
    else if (len == 7 && buffer[2] == 0x03 && buffer[3] == 0x02)
    {   // 天气情景模式设置色温亮度, 记录天气序号
        light_cb.mode = LIGHT_MODE_WEATHER;
        light_cb.weather_index = buffer[4];
        param1 = buffer[5] <= 100 ? buffer[5] : 100;
        param2 = buffer[6] <= 100 ? buffer[6] : (buffer[6] >= 1 ? buffer[6] : 1);
        service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x02, light_cb.weather_index, param1, param2 }, 7);
        light_cb.temperature_target = param1 * 100;
        light_cb.brightness_target = param2 * 100;
    }
    else if (len == 4 && buffer[2] == 0x03 && buffer[3] == 0x03)
    {   // 爆闪模式
        light_cb.mode = LIGHT_MODE_FLASH;
        service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x03 }, 4);
    }
    else if (len == 4 && buffer[2] == 0x03 && buffer[3] == 0x04)
    {   // 渐变模式
        light_cb.mode = LIGHT_MODE_FADE;
        service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x04 }, 4);
    }
    else if (len == 4 && buffer[2] == 0x03 && buffer[3] == 0x05)
    {   // SOS模式
        light_cb.mode = LIGHT_MODE_SOS;
        service_notify_event((u8[]) { 0x01, 0xF9, 0x03, 0x05 }, 4);
    }
    else if (len == 4 && buffer[2] == 0x03 && buffer[3] == 0x06)
    {   // DRL模式
...
{
    if (main_loop_printf_update == 0) return;
